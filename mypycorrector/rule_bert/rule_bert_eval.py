...
from mypycorrector.rule_bert import rule_bert_corrector

bertCorrector = bert_corrector.BertCorrector()
ruleBertCorrector=rule_bert_corrector.RuleBertCorrector()
...
eval_result= os.path.join(pwd_path,'./eval_result/sighan_rule_bert_char_eval_result.txt')
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if not error_sentence:
                continue
            # bert_pred_sentence, bert_pred_detail = bertCorrector.correct(error_sentence)
            pred_sentence,pred_detail=ruleBertCorrector.correct(error_sentence)
            if verbose:
                print('input sentence:', error_sentence)
                # print('bert pred sentence:', bert_pred_sentence, bert_pred_detail)
                print('rule bert pred sentence', pred_sentence, pred_detail)
