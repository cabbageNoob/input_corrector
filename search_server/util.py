...
def pinyin2hanzi(pinyin_list):
    ...
    if len(pinyin_list)<=2:
        ...
        while(2*i<len(pinyin_list)):
            ...
            i+=1
            path_score=dict(sorted(posting_data[pinyin_cur].items(),key=lambda x:x[1],reverse=True))
            value_hanzi_list.setdefault(pinyin_cur, [])
            value_hanzi_list[pinyin_cur] = path_score.keys()
        candidates=get_candidates(['*'*len(''.join(pinyin_list))],value_hanzi_list)
        scores=sentence_score(candidates)
        candidate_scores={candidate:score for candidate, score in zip(candidates,scores)}
